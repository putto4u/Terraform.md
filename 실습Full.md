## 📚 하이브리드 본사-3대륙 하이브리드 클라우드 인프라 구축 IAC 프로젝트 : hy3KUSE-pj

이 실습 프로젝트는 **Terraform**을 사용하여 **온프레미스 (VirtualBox) 환경의 마스터 노드**와 **AWS 클라우드 멀티-리전 (서울, 버지니아, 프랑크푸르트)의 워커 노드**(3대륙)를 통합하는 하이브리드 Kubernetes 인프라를 구축하는 것을 목표로 합니다. 이 프로젝트는 매우 전문적이고 고난이도의 프로젝트이나, 그럼에도 불구하고 실전에선 각 대륙별 더 많은 AZ, VM, 보안, 통제 시스템 등이 추가됩니다.
실습할 때는 최대한 직접 타이핑을 연습하셔야 자기 것으로 만들 수 있습니다. 에러가 많이 날수록 트러블슈팅 능력을 키울 수 있습니다. 두려워하지 마시고 끝까지 완수해 보세요. ^^
모두에게 행운을 빕니다!!!
